class_name DropBehaviorReturn extends DropBehavior
...
func evaluate(zone: DropZone, dropped_area: Area2D) -> DropPlan:
	...
	# If someone is there, send them back to where they came from
	if plan.drop_target and plan.drop_target.occupant and plan.drop_target.occupant != dropped_area:
		plan.actions.append(ActionReturn.new(plan.drop_target.occupant))
