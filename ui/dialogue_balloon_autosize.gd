...
@onready var speaker: Label = $PositionControl/PanelContainerBig/HBoxContainer/VBoxContainer/Speaker
@onready var speaker_text: RichTextLabel = $PositionControl/PanelContainerBig/HBoxContainer/VBoxContainer/SpeakerText
@onready var response_tpl: Button = $PositionControl/PanelContainerBig/HBoxContainer/VBoxContainer/ResponseTemplate
@onready var next_button: Button = $PositionControl/PanelContainerBig/HBoxContainer/NextButton

@onready var position_control = $PositionControl
@onready var arrow_right_control: Control = $PositionControl/PanelContainerBig/ArrowRightControl
@onready var arrow_left_control: Control = $PositionControl/PanelContainerBig/ArrowLeftControl
...
func _ready() -> void:
	...
	## Moving the Balloon to the correct position
	var marker = get_tree().root.find_child(character_name + "_Balloon_Pos", true, false)
	
	if is_instance_valid(marker):
		var position = marker.global_position
		#NPCs stay on the right
		if character_name != "Holly" and character_name != "WOM":
			position.x -= 400
			arrow_left_control.visible = false
			arrow_right_control.visible = true
		else:
			arrow_left_control.visible = true
			arrow_right_control.visible = false
		position_control.set_position(position)
	else:
		print("Couldn't find Balloon Pos for " + character_name)
